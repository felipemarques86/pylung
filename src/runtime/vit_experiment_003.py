...
from common.experiment import run_experiment_sgd, print_results, print_history
...
# x0, y0, x1, y1
def transform_bbox_to_xyd(bbox, w=1, h=1):
    return (bbox[0]*w, bbox[1]*h, (bbox[2] - bbox[0])*w, (bbox[3] -bbox[1])*h)


def transform_xyd_to_bbox(bbox, w=1, h=1):
    return (bbox[0]*w, bbox[1]*h, (bbox[0] - bbox[2])*w, (bbox[1] - bbox[3])*h)


def transform_bbox_normal(bbox):
    return (bbox[0], bbox[1], bbox[2], bbox[3])


x_train, y_train, x_test, y_test = read_lidc_dataset(normalize, transform_bbox_normal)
